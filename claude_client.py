...
SYSTEM_PROMPT = """
Ты пишешь подпись к сторис Pandora34. Это не описание фото.

Запрещено перечислять предметы в кадре. Зрителю и так видно фото.

Пиши про РАБОТУ — что чиним. Одно короткое предложение + второе ещё короче. Вместе максимум 90 знаков, чтобы влезло в ДВЕ строки сторис. Живо, чуть дерзко, без кринжа и без рекламы.

Тема мастера = какая работа. Не копируй её дословно. Не договаривай «соберём и…» — фраза должна быть законченной.

Запрещены: приехал на, чёрная магия, работает как надо, ещё один день, доверьте, хештеги, эмодзи.

Плохо: длинный абзац про пробег и «скоро соберём и»
Хорошо: «Щиток VW вскрыли. К вечеру поедет»
Хорошо: «Блок подушек. Ищем, где откис»
...
async def generate_post(photo_bytes: bytes | list[bytes], caption: str | None = None) -> str:
    ...
    if user_text:
        content.append(
            {
                ...
                "text": (
                    f"Работа: {user_text}\n"
                    "Не описывай фото. Две короткие фразы про работу, максимум 90 знаков, обе законченные."
                ),
            }
        )
    else:
        content.append(
            {
                "type": "text",
                "text": "Темы нет. Не описывай предметы на фото. Две короткие фразы, что за работу делают. Максимум 90 знаков.",
            }
        )
